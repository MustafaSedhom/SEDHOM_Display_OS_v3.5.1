...
//         if (ID == 0x9327 && orientation == LANDSCAPE) orientation = 3;
    ...
    void set_Round_Of_Shapes(int raduis){_radius = raduis;}
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //SEDHOM_Update
    void drawRect(int x, int y, int h, int w,Color_t color) 
    {
        MCUFRIEND_kbv::drawRect(x, y, w, h, color);
    }
    void drawRoundRect(int x, int y, int h, int w,int r,Color_t color) 
    {
        MCUFRIEND_kbv::drawRoundRect(x, y, w, h, r, color);
    }
    void fillRect(int x, int y, int w, int h,Color_t color) 
    {
        MCUFRIEND_kbv::fillRect(x, y, w, h, color);
    }
    void fillRoundRect(int x, int y, int h, int w,int r,Color_t color) 
    {
        MCUFRIEND_kbv::fillRoundRect(x, y, w, h, r, color);
    }
    void drawCircle(int x, int y, int radius,Color_t color) 
    { 
        MCUFRIEND_kbv::drawCircle(x, y, radius, color);
    }
    void fillCircle(int x, int y, int radius,Color_t color)
    {
         MCUFRIEND_kbv::fillCircle(x, y, radius, color);
    }
    void drawPixel(int x, int y, Color_t color) 
    {
         MCUFRIEND_kbv::drawPixel(x, y, color);
    }
    void drawLine(int x1, int y1, int x2, int y2,int color) 
    {
         MCUFRIEND_kbv::drawLine(x1, y1, x2, y2,color);
    }
    void fillTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color)
    {
        MCUFRIEND_kbv::fillTriangle( x0,  y0,  x1,  y1,  x2,  y2,  color);
    }
    void drawTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color)
    {
        MCUFRIEND_kbv::drawTriangle( x0,  y0,  x1,  y1,  x2,  y2,  color);
    }
    void setTextSize(byte size) 
    {
        MCUFRIEND_kbv::setTextSize(size); 
    }
    void print(char * str)
    {
        MCUFRIEND_kbv::print(str);
    }
    ...
    void drawRoundRect(int x1, int y1, int x2, int y2) {
        ...
        if (w < 0) { x1 = x2; w = -w; }
        if (h < 0) { y1 = y2; h = -h; }
        MCUFRIEND_kbv::drawRoundRect(x1, y1, w, h, 10, _fcolor);
    }
